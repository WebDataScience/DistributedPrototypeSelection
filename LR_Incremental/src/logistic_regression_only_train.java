...
public class logistic_regression_only_train {
...
public static void make_vectors (String col_att, String train, String target_variable,String model) throws IOException {
...
//URI path = URI.create("file:/tmp/vec.seq");
 ...
 hadoop_conf.set("fs.default.name","hdfs://turris/");

 hadoop_conf.set("mapred.job.tracker", "turris:9000");
  ...
  fs = FileSystem.get(hadoop_conf);
   ...
   System.out.println(fs.getUri());
    ...
    writer = SequenceFile.createWriter(fs, hadoop_conf,new Path(path1), IntWritable.class,VectorWritable.class);
      ...
      //learningAlgorithm = ModelSerializer.readBinary(new FileInputStream(model), OnlineLogisticRegression.class);
      ...
      System.out.println(reader_test_data);
      ...
      writer_test_name = SequenceFile.createWriter(fs, hadoop_conf,new Path(path3), 
				...
				*/
				...
				//make_vectors("connect-4-att.txt","connect-4.txt","vec_connect-4.txt", "43", "model");

				make_vectors(args[0],args[1],args[2],args[3]);
